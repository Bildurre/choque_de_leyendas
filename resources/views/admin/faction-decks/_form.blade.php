...
<form action="{{ $submitRoute }}" method="POST" enctype="multipart/form-data" id="faction-deck-form" class="form">
  ...
  <x-form.card :submit_label="$submitLabel" :cancel_route="route('admin.faction-decks.index')">
    ...
    <div class="form-section">
      <h2 class="form-section-title">{{ __('faction_decks.deck_config_info') }}</h2>
      
      <div class="form-info">
        <p>{{ __('faction_decks.selected_game_mode') }}: <strong>{{ $gameMode->name ?? (isset($factionDeck) ? $factionDeck->gameMode->name : '') }}</strong></p>
        
        @if(isset($deckConfig))
          <div class="deck-config-details">
            <p>{{ __('faction_decks.deck_config_details') }}:</p>
            <ul>
              <li>{{ __('faction_decks.min_cards') }}: <strong>{{ $deckConfig->min_cards }}</strong></li>
              <li>{{ __('faction_decks.max_cards') }}: <strong>{{ $deckConfig->max_cards }}</strong></li>
              <li>{{ __('faction_decks.max_copies_per_card') }}: <strong>{{ $deckConfig->max_copies_per_card }}</strong></li>
              <li>{{ __('faction_decks.max_copies_per_hero') }}: <strong>{{ $deckConfig->max_copies_per_hero }}</strong></li>
              <li>{{ __('faction_decks.required_heroes') }}: <strong>{{ $deckConfig->required_heroes }}</strong></li>
            </ul>
          </div>
        @else
          <p class="form-warning">{{ __('faction_decks.no_deck_config') }}</p>
        @endif
      </div>
    </div>
    ...
    <div class="form-section">
      <h2 class="form-section-title">{{ __('faction_decks.basic_info') }}</h2>
    ...
    <div class="form-section">
      <div class="deck-stats">
        <div class="deck-stats__header">
          <h3 class="deck-stats__title">{{ __('faction_decks.deck_stats') }}</h3>
        </div>
        
        <div class="deck-stats__grid">
          <div class="deck-stats__item">
            <span class="deck-stats__label">{{ __('faction_decks.cards') }}:</span>
            <span class="deck-stats__value deck-stats__cards">0/{{ $deckConfig->min_cards }}-{{ $deckConfig->max_cards }}</span>
          </div>
          
          <div class="deck-stats__item">
            <span class="deck-stats__label">{{ __('faction_decks.heroes') }}:</span>
    ...
    <div class="form-section">
      <h2 class="form-section-title">{{ __('faction_decks.cards') }}</h2>
      
      <x-form.entity-selector
        name="cards"
        :label="__('faction_decks.select_cards')"
        :entities="$allCards"
    ...
    <div class="form-section">
      <h2 class="form-section-title">{{ __('faction_decks.heroes') }}</h2>
      
      <x-form.entity-selector
        name="heroes"
        :label="__('faction_decks.select_heroes')"
        :entities="$allHeroes"
