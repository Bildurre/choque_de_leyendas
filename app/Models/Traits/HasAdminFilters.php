...
trait HasAdminFilters
{
    ...
    protected function applySearchFilters(Builder $query, Request $request): void
    {
        ...
        if ($request->has('search') && !empty($request->search) && method_exists($this, 'getAdminSearchable')) {
            ...
            $query->where(function (Builder $query) use ($searchTerm, $searchable) {
                foreach ($searchable as $field) {
                    ...
                    if (Str::contains($field, '.')) {
                        ...
                        // Regular field with case-insensitive search
                        $query->orWhereRaw('LOWER(' . $field . ') LIKE ?', [strtolower($searchTerm)]);
                    }
                }
            });
        }
    ...
    /**
     * Apply column filters to the query (placeholder for future implementation)
     *
     * @param Builder $query
     * @param Request $request
     * @return void
    ...
    protected function applyColumnFilters(Builder $query, Request $request): void
    {
        ...
        // when we add back the column filters functionality
    }
    
    /**
     * Get the filter parameter name for a field
     *
     * @param string $field
     * @return string
     */
    protected function getFilterParamName(string $field): string
    {
        // Convert relation.field to relation_field for URL parameters
        return str_replace('.', '_', $field) . '_filter';
    }
}